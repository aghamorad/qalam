...
def draw_page(page_no: int):
    ...
    painter.drawText(QRectF(55, 55, 485, 50), flags,
                     f"فصل {page_no}: آزمون تبدیل قلم")

    painter.setFont(body)
    prose = (
        "این یک متن فارسی برای آزمون واقعی استخراج و تبدیل است. "
        "قلم باید ترتیب واژه‌ها، اعداد ۱۲۳۴ و نشانه‌گذاری را حفظ کند. "
        "این فایل در خود فرایند CI ساخته می‌شود تا آزمون به یک کتاب خصوصی یا "
        "فایل بیرونی وابسته نباشد. "
    ) * 4
    painter.drawText(QRectF(55, 130, 485, 390), flags, prose)

    painter.setFont(note)
    painter.drawText(QRectF(55, 700, 485, 60), flags,
                     f"{page_no}. این یک پانوشت کوچک برای آزمون تشخیص پانوشت است.")
